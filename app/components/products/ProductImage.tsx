...
const ProductImage = ({ product }) => {
    ...
    return (
        <div className="grid grid-cols-6 gap-2 h-full max-h-[500px] min-h-[300px] sm:min-h-[400px]">
            <div className="col-span-1 flex flex-col items-center max-sm:h-[285px] max-md:h-[330px] max-lg:h-[300px]">
                ...
                <div className="overflow-hidden max-h-[300px]" ref={scrollContainer} onScroll={checkScrollPosition}>
                    <div className="flex flex-col h-full">
                        {product.images.map((image, index) => (
                            <div key={index} onClick={() => setSelectedImage(image.image)}
                                 className={`cursor-pointer p-1 mb-2 ${selectedImage === image.image ? 'border-[2px] box-sizing' : ''}`}>
            ...
            <div className="col-span-5 relative aspect-square">
                <Image src={selectedImage} alt="Selected product image" layout="fill" className="object-contain object-top"/>
            </div>
        </div>
    );
};
